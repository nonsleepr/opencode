...
You can reference files and other resources in your messages using `@`. This does a fuzzy search in the current working directory.

:::tip
Type `@` followed by a filename to fuzzy search and attach files to your prompt. You can also reference other resources like agents, sessions, and more through plugins.
:::
...
You can use `@` to reference agents in your prompts:

```text
@explore find authentication code in the API
```

### Reference line ranges

Specify line ranges when referencing files:

```text
@src/app.ts#10-20
```

This includes only lines 10-20 from the file.

### Plugin resources

Plugins can add custom resource types that appear in `@` autocomplete. See [Resources](/docs/resources) for details.
