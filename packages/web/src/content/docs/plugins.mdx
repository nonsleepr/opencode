...
Plugins can extend the `@` reference system by providing custom resource types. This allows you to reference sessions, documentation, database schemas, or any other contextual data in your prompts.
  ...
  return {
    "resource.provider": {
      ...
      async search(query: string, limit: number) {
        const sessions = await ctx.client.session.list({ limit: 50 })
        return sessions
          .filter((s) => s.title?.toLowerCase().includes(query.toLowerCase()))
          .slice(0, limit)
          .map((s) => ({
            uri: `opencode://session/${s.id}`,
            name: s.title ?? s.id,
            mimeType: "text/plain",
            metadata: {
              subtitle: `[sessions] · ${formatDate(s.createdAt)}`,
            },
          }))
      },
...
After installing this plugin, you can reference sessions in prompts using `@session-title`, and they'll appear in autocomplete as `Session Title · [sessions] · timestamp`.

Learn more in the [Resources](/docs/resources) documentation.
